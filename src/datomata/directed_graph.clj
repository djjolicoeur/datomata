(ns datomata.directed-graph
  (:require [datomata.node :as node]
            [loom.graph :as graph]
            [loom.alg   :as alg]))
...
(defrecord DatomataGraph [g nodes]
  Graph
  (graph [this] g)
  (nodes [this] nodes))

(defn new-graph [g nodes]
  (->DatomataGraph g nodes))
...
(defn make-graph
  ...
  {:pre
   [(> (count (filter node/accept? (vals node-map))) 0)
    (instance? datomata.node.StartNode (:start node-map))]}
  (let [nodes (assoc node-map :rej node/reject-node)
    ...
    (for [k (keys nodes)]
      (when-not (node/terminal-node? (get nodes k))
        (assert
         (->> accept-nodes
              (map (partial alg/shortest-path graph k))
              (some identity)))))
